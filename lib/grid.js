...
class Grid {
  ...
  positionStartOfShip(ship){
    ...
    this.getCurrentState()[convertedLetterToNumber][startColumn - 1] = 'O'
  }
  positionEndOfShip(ship){
    const endRow = ship.getGridPosition()[1].split("")[0]
    const convertedLetterToNumber = this.#convertLetterToNumber(endRow)
    const endColumn = ship.getGridPosition()[1].split("")[1]
    this.getCurrentState()[convertedLetterToNumber][endColumn - 1] = 'O'
  }
